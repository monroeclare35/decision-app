...
export function BeliefGuidance() {
  ...
  return (
    <div className="rounded-xl border border-sage-200 bg-white p-4">
      ...
      >
        <span className="font-medium text-sage-600">💡 什么是核心信念？</span>
        <span className="text-xs text-sage-400">{show ? '收起 ▲' : '了解更多 ▼'}</span>
      </button>
      {show && (
        <div className="mt-3 space-y-2 text-sm leading-relaxed text-sage-600">
          <p>
            核心信念是你确立的、长期有效的个人原则。它们会<strong>强制融入</strong>每一次 AI
            决策分析中。
          </p>
          <p className="font-medium">好的例子：</p>
          <ul className="ml-4 list-disc space-y-1 text-sage-500">
