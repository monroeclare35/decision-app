...
export function SimilarCases({ cases }: SimilarCasesProps) {
  return (
    <div className="card rounded-xl bg-white p-5">
      <h3 className="mb-3 text-sm font-semibold text-surface-600">📖 相似情境参考</h3>
      <div className="space-y-3">
        {cases.map((c, i) => (
          <div key={i} className="rounded-lg border border-surface-100 p-3">
            <p className="text-sm font-medium text-surface-700">{c.scenario}</p>
            <p className="mt-1 text-xs leading-relaxed text-surface-500">{c.outcome}</p>
